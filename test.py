...
def testavi():
    ...
    test2.window.filename = filedialog.askopenfilename(initialdir='', title='파일선택', filetypes=(
    ('mp4 files', '*.mp4'), ('png files', '*.png'), ('jpg files', '*.jpg'), ('all files', '*.*')))

    Label(test2.window, text=test2.window.filename).pack() # 파일경로 view
    # print(test2.window.filename);

    cap = cv2.VideoCapture(test2.window.filename)
